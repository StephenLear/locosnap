...
- **Submitted both stores** via `eas submit`: **iOS build 60 uploaded to App Store Connect (processing)** — still needs the manual ASC steps (select build → release notes → EULA → Submit for Review); **Android versionCode 30 in Google Play as a production DRAFT** — needs release notes + rollout in the Play Console.
