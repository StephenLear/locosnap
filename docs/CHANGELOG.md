...
#### `backend/src/services/trainSpecs.ts` — BR 159 (Vectron Dual Mode) spec entry added
- **Added** KNOWN_SPECS keys for **BR 159** (the Captrain / TX Logistik private-operator designation for the Siemens Vectron Dual Mode), which had no entry and so fell through to AI generation — showing plain "Diesel" with no weight. Now locks fuelType to **Dual-Mode (15 kV 16.7 Hz AC overhead + Diesel)**, weight **90 t**, 160 km/h, 2,610 kW (electric) / 2,000 kW (diesel). Reported via the one-month feedback DM by a Captrain BR 159 driver-in-training who flagged the missing dual-mode fuel label, weight, and length. **The tester's "123 t" figure was deliberately not used** — physically impossible on a 4-axle Bo'Bo' (~30 t/axle); Siemens lists a 90 t service weight (verified). Loco **length** is not a displayed spec field (`SpecsOverride` has no `length` key) — logged as a frontend feature request, not a quick fix. 202/202 backend tests pass, typecheck clean. **Not yet deployed — needs a push to go live on Render.**
