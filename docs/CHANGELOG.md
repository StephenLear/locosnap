...
- **`locales/{en,de,pl}.json`** — added `tabs.radar` + a `radar.*` block (subtitle, grid labels, empty, info-card labels, sign-in gate strings) in all three languages.
- **Sign-in gate (added this session, before push):** Radar is gated behind sign-in — the heatmap is communal collection data and the RPC is `authenticated`-only, so giving it to signed-out users would hand the feature to non-registered users. Signed-out users now see a teaser card (radio icon + "Sign in to explore Train Radar" + body) with **Log In / Sign Up** CTAs routing to `/sign-in?mode=login` and `/sign-in?mode=signup` (mirrors the Profile tab's three-CTA pattern). The tab stays visible to all (creates sign-up pull); the auth-only RPC is not called when signed out. Map renders only for signed-in users.
