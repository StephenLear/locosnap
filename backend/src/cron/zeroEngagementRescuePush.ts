...
export interface CandidateRow {
  id: string;
  // profiles.language added in migration 017 (2026-05-26) — synced
  // from frontend settingsStore on every language change, with backfill
  // from country_code for pre-migration rows. NOT NULL + CHECK
  // constraint at the DB layer guarantees this is always one of
  // 'en' / 'de' / 'pl'. Pre-migration commit a71fdd5 used a country_code
  // → language runtime mapper as a stopgap; migration 017 retires it.
  language: string;
