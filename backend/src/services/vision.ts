...
- VR Finland fleet disambiguation — Sm3 (Pendolino) vs Dm12 and other Finnish rolling stock: Finnish state railway VR operates a distinctive fleet. The most visually striking unit is the **VR Sm3 (Pendolino)**: a TILTING high-speed EMU built by Fiat Ferroviaria (later Alstom) from 1995, based on the ETR 460 platform. Key visual identifiers: white body with red and blue horizontal stripes running along the entire train length; PANTOGRAPH(S) visible on the roof (ELECTRIC — not diesel); distinctive AERODYNAMIC TILTING NOSE that tapers to a sharp point with an elongated streamlined profile; 6-car articulated formation; bogie fairings are present to accommodate the tilt mechanism; operated on Helsinki–Turku, Helsinki–Tampere, and Helsinki–Joensuu routes at up to 220 km/h. The Sm3 is frequently nicknamed "Pendolino" and is instantly recognisable by its sleek white/red/blue livery and pointed nose. The **VR Dm12**: an older DIESEL multiple unit (no pantograph), boxy 1980s-era design, used on low-traffic regional and branch-line services — entirely different body profile and clearly pre-modern in appearance. The **VR Sm5 (Flirt)**: a Stadler FLIRT EMU operated by VR for commuter and regional services — covered by the FLIRT disambiguation rule elsewhere. The **VR Sr3**: a Siemens Vectron electric LOCOMOTIVE (single traction unit that hauls separate coaches — it is NOT an EMU). CRITICAL RULES: (1) If a train in Finland has a white/red/blue livery, a pointed aerodynamic nose, pantographs, and an articulated 6-car formation, it is ALMOST CERTAINLY the VR Sm3 (Pendolino) — do NOT classify it as Dm12 (diesel, different body) or any German DMU. (2) Dm12 is a diesel — it has NO pantograph. If pantographs are visible, it cannot be Dm12. (3) The Sm3 and Dm12 are visually very different — the Sm3 is a modern high-speed tilting electric train; the Dm12 is an older boxy diesel. Never confuse these two classes.`;
