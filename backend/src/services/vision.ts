...
- NS Dutch EMU family disambiguation (ICNG vs VIRM vs SLT vs SGM): NS (Nederlandse Spoorwegen) operates several visually distinct EMU families, all in the same yellow/ochre NS livery. They MUST be distinguished carefully. **ICNG (Intercity New Generation / NS Materieel 2024)**: brand-new Alstom Avelia Horizon EMU entering service from 2024–2025. The most modern NS train by far — SINGLE-DECK with a very distinctive SHARP, ANGULAR, AERODYNAMIC NOSE with a pronounced V-shaped front profile, large panoramic cab windscreen that wraps slightly, smooth clean bodysides with large passenger windows, and prominent LED headlight clusters. The cab nose tapers to a clearly aerodynamic sharp point, unlike the flat-fronted VIRM. The NS double-arrow logo is visible. Max speed ~200 km/h. This is a completely new generation of NS intercity train and must be identified as "ICNG" (not VIRM, not SLT). **VIRM (Verlengd Interregio Materieel)**: the long-running NS DOUBLE-DECK intercity EMU — unmistakably double-deck (two rows of windows stacked vertically on the sides), flat-fronted cab with the characteristic angular-but-flat VIRM nose profile (not aerodynamically sharp), yellow/grey bodywork on older variants or yellow with blue band on newer variants. Max speed 160 km/h. Built by Bombardier. If two rows of windows are clearly visible on the coach body, it is a VIRM. **SLT (Sprinter Lighttrain)**: NS regional Sprinter EMU, single-deck, shorter formation, used on S-Bahn/Sprinter commuter services — the SLT has a distinctly different, lower-profile cab nose compared to the ICNG. **SGM (Sprinter)**: older NS Sprinter family, boxy cab design. CRITICAL RULES: (1) If the NS train is DOUBLE-DECK (two stacked rows of windows on the coach sides), it is the VIRM — it cannot be ICNG or SLT. (2) If the NS train is SINGLE-DECK and has a SHARP, MODERN, ANGULAR AERODYNAMIC NOSE with a V-shaped profile, it is the ICNG — it is NOT a VIRM, NOT a BR 186, and NOT an SLT. (3) BR 186 (Bombardier TRAXX electric locomotive) is a SINGLE-UNIT FREIGHT/PASSENGER LOCOMOTIVE — it has a completely different boxy loco body, does NOT have a passenger saloon, and does NOT look like any NS passenger EMU. If you are identifying an NS multiple-unit passenger train, it cannot be a BR 186. (4) The ICNG entered service in 2024–2025 and is brand new — it may have limited AI training data, so rely heavily on the sharp aerodynamic nose profile and single-deck modern bodyside design.
