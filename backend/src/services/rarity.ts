...
- British Rail Class 43 (HST) rarity: 197 power cars built 1975–1982 (BREL Crewe). The general Class 43 HST still runs with ScotRail (Inter7City), charter operators, and recently CrossCountry — classify a standard Class 43 / InterCity 125 power car as **"uncommon"** (a recognisable but no-longer-everyday survivor of the HST era). **EXCEPTION — Network Rail New Measurement Train (NMT) "Flying Banana":** a Class 43 in all-over Network Rail YELLOW livery (the NMT power cars 43013 / 43014 / 43062) is a UNIQUE 3-power-car measurement set — classify it as **"rare"**, NOT uncommon. It is a genuine trophy spot (irregular nationwide measurement schedule, turns up unannounced, carries no passengers). Do NOT rate the yellow NMT "common"/"uncommon" (under-rates a one-of-a-kind set), and do NOT rate it "epic"/"legendary" (it is a working train in regular use, not heritage/near-extinct, and ~50+ general Class 43s still survive). Reason field: general 43 = uncommon HST survivor; Network Rail Yellow = the unique NMT trophy. Added 2026-06-30 (tester Steph the Spotter NMT footage).
- VR Sr-class rarity: three generations of VR electric locomotive with very different rarity profiles. **VR Sr1**: 112 units built 1973–1985 by Novocherkassk (NEVZ) + Strömberg (Bo'Bo' four-axle, max 140 km/h — NOT Co'Co'/six-axle or 160; corrected 2026-06-30). Being actively withdrawn as Sr3 Vectrons replace it — active fleet is shrinking. Classify as "rare" (heading toward "epic" as withdrawal continues through 2026–2028). The classic red-green-yellow Finnish tricolor livery is especially rare now; if that livery is visible, lean toward "epic". **VR Sr2**: only 46 units ever built (1995–2003 by SLM Winterthur / ABB, Re 460 family) — ALL still in service. Small fleet by international standards but reliable mainline presence on VR intercity routes. Classify as "rare" — a 46-unit fleet is genuinely small, but not vanishing. **VR Sr3**: 80 units for VR (all in service) + 3 for private operator Fenniarail Oy (numbered 201-203) = 83 total, from Siemens 2017+ — the modern mainline backbone, actively growing. Classify as "uncommon" — it is the new standard Finnish mainline electric, so spotters encounter them often but the absolute fleet is still modest. Do NOT classify Sr3 as "common" (fleet is not large enough) or "rare" (it is now everyday infrastructure on VR mainlines).
- VR Finnish diesel rarity — Dr16 vs Dr19 (added 2026-06-24, tester Oula): **VR Dr16** ("Iso Vaalee") — only **23 ever built** (1985–1992), and VR has now **withdrawn the entire fleet** (the last VR units retired by 2026, ~11 scrapped, none left with VR). The class survives ONLY with the private operator **ArcticRail**, which runs just **two** restored units — these are the only Dr16s operating anywhere. Classify Dr16 as **"epic"** — a near-extinct class with only a handful operational, comparable to BR 143 / BR 485 in active-fleet terms. Do NOT classify as "common", "uncommon", or "rare". **VR Dr19** — Stadler's new mainline diesel, **60 being delivered through 2026**, already the standard VR diesel-freight loco and a frequent everyday sight at Finnish stations. Classify Dr19 as **"uncommon"** (modern, growing, but a single-country 60-unit fleet — same logic as the Sr3). Do NOT classify Dr19 as "rare" (it is becoming everyday) or "common" (the fleet is still modest). Do not confuse the two: Dr16 = old, near-gone, epic; Dr19 = new, growing, uncommon.
...
const KNOWN_RARITY: Record<string, { tier: RarityTier; reason?: string }> = {
  ...
  "en71": { tier: "uncommon" },
  "dr18": { tier: "legendary" },
  // VR Dr16 near-extinct (23 built, VR fleet retired by 2026, a few with ArcticRail)
  // = epic; VR Dr19 modern growing Stadler fleet (60 by end 2026) = uncommon.
  // Added 2026-06-24 (tester Oula). Bare + "vr " forms to match returned class names.
  "dr16": { tier: "epic" }, "vr dr16": { tier: "epic" }, "arcticrail dr16": { tier: "epic" },
